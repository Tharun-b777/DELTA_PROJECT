...
def modify_office(id):
    ...
    office=post_office.query.get(id)
    if office:
        office.Officename = request.get_json().get('Officename', office.Officename)
        office.Pincode = request.get_json().get('Pincode', office.Pincode)
        office.Officetype = request.get_json().get('Officetype', office.Officetype)
        office.Dstats = request.get_json().get('Dstats', office.Dstats)
        office.Divname = request.get_json().get('Divname', office.Divname)
        office.Regname = request.get_json().get('Regname', office.Regname)
        office.Circlename = request.get_json().get('Circlename', office.Circlename)
        office.Taluk = request.get_json().get('Taluk', office.Taluk)
        office.Distname = request.get_json().get('Distname', office.Distname)
        office.Statename = request.get_json().get('Statename', office.Statename)
        office.Phnum = request.get_json().get('Phnum', office.Phnum)
        office.RHO = request.get_json().get('RHO', office.RHO)
        office.RSO = request.get_json().get('RSO', office.RSO)
        db.session.commit()
        return post.jsonify(office)
    else:
        return jsonify({'data':"not_existing"})
